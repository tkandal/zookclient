...
func (z *ZooKeeperClient) GetChildren(root string) ([]string, error) {
	is, err := exists(z.curator, root)
	if !is || err != nil {
		return nil, err
	}
	paths := make([]string, 0)
	if err = z.getChildren(root, &paths); err != nil {
		return nil, err
	}
	return paths, err
}

func (z *ZooKeeperClient) getChildren(root string, paths *[]string) error {
	children, err := z.curator.GetChildren().ForPath(root)
	if err != nil {
		return err
	}
	if len(children) > 0 {
